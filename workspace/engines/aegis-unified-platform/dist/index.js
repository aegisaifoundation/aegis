...
export class UnifiedPlatformEngine {
    metadata = {
    ...
    async start() {
        ...
        const event = EventSpecification.createEvent('NodeStarted', 'aegis-unified-platform', { status: 'ACTIVE' }, { priority: 'HIGH' });
    ...
    async shutdown() {
        ...
        const event = EventSpecification.createEvent('NodeStopped', 'aegis-unified-platform', { status: 'STOPPED' }, { priority: 'HIGH' });
        EventSpecification.publishEvent(this.context.getEventBus(), event);
