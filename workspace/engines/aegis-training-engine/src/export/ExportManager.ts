...
import { TrainingJob, ExportMetadata } from '../types/index.js';

export class ExportManager {
  private activeBackend: ITrainingBackend;
  private workspaceRoot: string;

  constructor(backend: ITrainingBackend, workspaceRoot: string = process.cwd()) {
  ...
  async ExportLoRA(job: TrainingJob, exportName: string): Promise<string> {
  ...
  private computeSignature(jobId: string, stats: any): string {
    const raw = `${jobId}-${JSON.stringify(stats)}`;
    return crypto.createHmac('sha256', 'aegis-secret-key').update(raw).digest('hex');
  }

  private registerInAiRuntime(loraId: string, baseModelId: string) {
  ...
  private publishToDistributedLearning(loraId: string, metadata: ExportMetadata) {
    if (serviceRegistry.has('aegis-distributed-learning')) {
      const dle = serviceRegistry.get<any>('aegis-distributed-learning');
