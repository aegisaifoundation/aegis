import type { TrainingMetrics, LoRAAdapter, LoRAConfig } from '../types/index.js';
...
export class MockNode {
  ...
  private localMemory: Record<string, any>;
   ...
   */
  trainLocally(epochs = 2): Record<string, number[]> {
    const lr = 0.01;
    for (let e = 0; e < epochs; e++) {
      for (const key of Object.keys(this.localWeights)) {
        this.localWeights[key] = this.localWeights[key]!.map(
          w => w - lr * (Math.random() - 0.5) * 0.05
        );
      }
