...
export class CollectiveIntelligenceSimulation {
  private nodes = new Map<string, CollectiveIntelligenceEngine>();
  ...
  async runSimulation(): Promise<{
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (domain === 'programming' || domain === 'legal') {
        nodeA.RecordExperience(
          taskId,
          `Simulated goal: Solve ${domain} task ${i}`,
          [`Trace step 1: Query ${domain} context`, `Trace step 2: Execute ${domain} tool`],
          [`tool_${domain}`],
          ['llama-3'],
          [domain],
      ...
      if (domain === 'medical') {
        nodeB.RecordExperience(
          taskId,
          `Simulated goal: Solve medical task ${i}`,
          ['Trace step 1: Retrieve clinical notes', 'Trace step 2: Check diagnostics'],
          ['Medical_OCR_Tool'],
          ['llama-3'],
          ['medical'],
          success ? 'success' : 'failure',
