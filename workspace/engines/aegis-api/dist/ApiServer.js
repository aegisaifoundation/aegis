    ...
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (pathname === '/api/sessions/rename' && req.method === 'POST') {
                ...
                await runtimeSessionManager.renameSession(sessionId, trimmedDisplayName, description ?? metadata?.description ?? '', 'user');
                res.setHeader('Content-Type', 'application/json');
                res.writeHead(200);
            ...
            if (pathname === '/api/capabilities/add' && req.method === 'POST') {
                ...
                let capType;
                if (type === 'tool')
                    capType = CapabilityType.TOOL;
                else if (type === 'skill')
                    capType = CapabilityType.SKILL;
                else if (type === 'plugin')
                    capType = CapabilityType.PLUGIN;
                else {
                    res.setHeader('Content-Type', 'application/json');
                    res.writeHead(400);
            ...
            if (pathname === '/api/chat' && req.method === 'POST') {
                ...
                // Event listeners
                const onExecutionStarted = (envelope) => sendEvent('execution_started', envelope ? envelope.payload : null);
                const onMessageReceived = (envelope) => sendEvent('message_received', envelope ? envelope.payload : null);
                const onThinkingStarted = () => sendEvent('thinking_started', {});
                const onThinkingFinished = () => sendEvent('thinking_finished', {});
                const onResponseChunk = (envelope) => sendEvent('response_chunk', { chunk: envelope ? envelope.payload : '' });
                const onToolStarted = (envelope) => sendEvent('tool_started', envelope ? envelope.payload : null);
                const onToolFinished = (envelope) => sendEvent('tool_finished', envelope ? envelope.payload : null);
                const onRuntimeError = (envelope) => sendEvent('runtime_error', { error: envelope ? envelope.payload : '' });
