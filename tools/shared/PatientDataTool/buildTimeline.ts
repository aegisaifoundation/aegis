...
export default async function execute(input: any, _context: ToolContext): Promise<string> {
  ...
  const withDates = encountersRaw.map((e: PatientEncounter, idx: number) => ({
    encounter: e,
  ...
  const gaps = timeline.map((t) => t.daysSincePrevious).filter((g): g is number => g !== null);
