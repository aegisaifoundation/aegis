...
export class MemoryCompressionManager {
  private static instance = new MemoryCompressionManager();

  public static getInstance(): MemoryCompressionManager {
    return this.instance;
  }

  public async compressHistory(sessionId: string, messages: Message[]): Promise<CompressedHistory> {
...
Output strictly valid JSON only:`;
        ...
        const summary = JSON.parse(cleaned) as CompressedHistory;
  ...
  private fallbackParse(messages: Message[]): CompressedHistory {
    const goals: string[] = [];
    const facts: string[] = [];
    ...
    if (goals.length === 0) goals.push("Maintain dialogue session state.");
    if (facts.length === 0) facts.push("Standard medical data processing guidelines.");
    if (decisions.length === 0) decisions.push("Initialized memory context.");
    if (risks.length === 0) risks.push("None detected.");
