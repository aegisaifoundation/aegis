...
export class BootstrapManager {
  async bootstrap(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(runtimeConfig.autoloadTools)) autoloadTools = runtimeConfig.autoloadTools;
      if (Array.isArray(runtimeConfig.autoloadCommands)) autoloadCommands = runtimeConfig.autoloadCommands;
      if (Array.isArray(runtimeConfig.autoloadPlugins)) autoloadPlugins = runtimeConfig.autoloadPlugins;
      if (Array.isArray(runtimeConfig.autoloadSkills)) autoloadSkills = runtimeConfig.autoloadSkills;
    } catch (e) {
    ...
    const commandLoadTask = (async () => {
      ...
      const results = await Promise.allSettled(
        autoloadCommands.map(async (cmdPath) => {
    ...
    const pluginLoadTask = (async () => {
      const results = await Promise.allSettled(
        autoloadPlugins.map((pluginPath) => capabilityManager.add(CapabilityType.PLUGIN, pluginPath))
      );
      for (const result of results) {
        if (result.status === 'rejected') {
    ...
    const skillLoadTask = (async () => {
      const results = await Promise.allSettled(
        autoloadSkills.map(async (skillPath) => {
    ...
    const toolLoadTask = (async () => {
      const results = await Promise.allSettled(
        autoloadTools.map((toolPath) => capabilityManager.add(CapabilityType.TOOL, toolPath))
      );
      for (const result of results) {
        if (result.status === 'rejected') {
