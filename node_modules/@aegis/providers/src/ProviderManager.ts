...
import { config, eventBus, configurationManager } from '@aegis/runtime';

export class ProviderManager {
  private activeProviderName: string = 'local/ollama';
  private fallbackProviderName: string = 'local/ollama';
  private _providerInstanceOverride: Provider | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    if (!active || providerRegistry.getProviderState(this.activeProviderName) !== ProviderState.READY) {
      ...
      const readyProviders = providerRegistry.listNames().filter(
        name => providerRegistry.getProviderState(name) === ProviderState.READY
      );
    ...
    const otherReadyProviders = providerRegistry.listNames().filter(
      name => name !== this.activeProviderName && providerRegistry.getProviderState(name) === ProviderState.READY
    );
  ...
  setProvider(provider: Provider | string) {
