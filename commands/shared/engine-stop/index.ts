...
export default {
  name: 'engine stop',
  description: 'Stop a running engine by its ID.',

  async execute(input: string, context: any): Promise<string> {
    const engineId = input.trim();
    if (!engineId) {
      return '  Usage: engine stop <engine-id>\n  Example: engine stop distributed-intelligence';
    }
    ...
    let resp: any;
    try {
      resp = await sendIpc(workspacePath, 'stopEngine', { engineId });
    } catch (err: any) {
    ...
    return `❌ ${resp.error ?? 'Failed to stop engine.'}`;
  }
};
