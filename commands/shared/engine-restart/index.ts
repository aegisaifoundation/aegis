...
export default {
  name: 'engine restart',
  description: 'Gracefully stop and restart an engine by its ID.',

  async execute(input: string, context: any): Promise<string> {
    const engineId = input.trim();
    if (!engineId) {
      return '  Usage: engine restart <engine-id>\n  Example: engine restart distributed-intelligence';
    }
    ...
    let resp: any;
    try {
      resp = await sendIpc(workspacePath, 'reloadEngine', { engineId });
    } catch (err: any) {
    ...
    return `❌ ${resp.error ?? 'Failed to restart engine.'}`;
  }
};
