...
export default async function execute(input: string, context: CommandContext): Promise<CommandResult> {
  ...
  if (args.length < 2) {
    return {
      success: false,
      message: 'Usage: /update <tool|plugin> <path>. Example: /update plugin shared/analytics'
    };
  }
  ...
  if (typeArg === 'tool') {
    ...
    return {
      success: false,
      message: `Error: Unsupported capability type '${args[0]}'. Allowed types are 'tool' or 'plugin'.`
    };
  }
