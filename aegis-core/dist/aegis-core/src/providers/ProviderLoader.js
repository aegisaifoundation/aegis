...
export class ProviderLoader {
    ...
    async loadProvider(providerPath) {
        const providersDir = this.getProvidersDir();
        const providerDir = path.resolve(providersDir, providerPath);
        if (!fs.existsSync(providerDir)) {
            throw new Error(`Provider directory not found: ${providerDir}`);
        }
        const metadataPath = path.join(providerDir, 'provider.json');
        if (!fs.existsSync(metadataPath)) {
            throw new Error(`provider.json not found in ${providerDir}`);
        }
        ...
        let indexPath = path.join(providerDir, entryName);
        if (!fs.existsSync(indexPath)) {
            const fallbackName = entryName.endsWith('.ts') ? entryName.replace(/\.ts$/, '.js') : entryName.replace(/\.js$/, '.ts');
            const fallbackPath = path.join(providerDir, fallbackName);
            if (fs.existsSync(fallbackPath)) {
                indexPath = fallbackPath;
            }
        }
        if (!fs.existsSync(indexPath)) {
            throw new Error(`Entry file not found in ${providerDir}`);
        }
        ...
        // Set fields from manifest if not present
    ...
    async initializeProvider(providerPath) {
