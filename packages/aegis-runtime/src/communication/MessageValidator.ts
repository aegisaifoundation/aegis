...
export class MessageValidator {
  static validateNodeId(nodeId: string, label: string = 'nodeId'): void {
    if (!nodeId || typeof nodeId !== 'string' || !nodeId.startsWith('aegis://')) {
      throw new CommunicationError(
        CommunicationErrorCode.INVALID_NODE_ID,
        `Invalid ${label}: "${nodeId}". Must be a canonical identity starting with "aegis://".`
      );
    }
  }
  ...
  static validateEnvelope(envelope: any): IAegisMessageEnvelope {
    ...
    if (envelope.protocolVersion !== AEGIS_NET_PROTOCOL_VERSION) {
      throw new CommunicationError(
        CommunicationErrorCode.PROTOCOL_MISMATCH,
        `Protocol version mismatch. Required: "${AEGIS_NET_PROTOCOL_VERSION}", received: "${envelope.protocolVersion}".`
      );
    }
    ...
    if (!envelope.messageId || typeof envelope.messageId !== 'string' || !envelope.messageId.startsWith('aegis-msg://')) {
      throw new CommunicationError(
        CommunicationErrorCode.INVALID_MESSAGE,
        `Invalid messageId: "${envelope.messageId}". Must start with "aegis-msg://".`
      );
    }
    ...
    if (!envelope.messageType || typeof envelope.messageType !== 'string' || envelope.messageType.trim() === '') {
      throw new CommunicationError(
        CommunicationErrorCode.INVALID_MESSAGE,
        'Message envelope missing valid messageType string.'
      );
    }
    ...
    if (typeof envelope.timestamp !== 'number' || envelope.timestamp <= 0) {
      throw new CommunicationError(
        CommunicationErrorCode.INVALID_MESSAGE,
        `Invalid timestamp: ${envelope.timestamp}`
      );
    }
    ...
    if (Date.now() > expiresAt) {
      throw new CommunicationError(
        CommunicationErrorCode.MESSAGE_EXPIRED,
        `Message "${envelope.messageId}" expired at ${new Date(expiresAt).toISOString()} (Current: ${new Date().toISOString()}).`
      );
    }
    ...
    return envelope as IAegisMessageEnvelope;
  }
}
