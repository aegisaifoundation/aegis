...
const getConversationContext = () => serviceRegistry.get<any>('conversationContext');
const getAgent = () => serviceRegistry.get<any>('agent');
...
export class RuntimeExecutor {
  ...
  async execute(userInput: string): Promise<void> {
    ...
    try {
      ...
      if (isTask) {
        try {
          const skillsList = getSkillRegistry().list().map((s: any) => `- ${s.name}: ${s.description}`).join('\n') || 'None';
          const toolsList = getToolRegistry().getAllTools().map((t: any) => `- ${t.name}: ${t.description}`).join('\n') || 'None';
...
Planned Tasks:
${(freshState.tasks || []).map((t: any) => `- ${t}`).join('\n') || 'None'}

Current Active Tasks (with statuses):
${freshState.activeTasks.map((t: any) => `- ${t}`).join('\n') || 'None'}
