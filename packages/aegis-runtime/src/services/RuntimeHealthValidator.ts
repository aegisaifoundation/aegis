...
import { RuntimeHealthStatus, SessionState } from '@aegis/sdk';

export class RuntimeHealthValidator {
  private static instance = new RuntimeHealthValidator();

  public static getInstance(): RuntimeHealthValidator {
   ...
   */
  public async validateHealth(): Promise<{ healthy: boolean; status: RuntimeHealthStatus; errors: string[] }> {
    const errors: string[] = [];
    ...
    if (activeSessionId) {
      ...
      if (!existsSync(sessionDir)) {
        ...
        try {
          sessionState = await getMemoryGateway().getSessionState(activeSessionId);
        ...
        if (sessionState) {
          try {
            const workingMemory = await getMemoryGateway().getWorkingMemory(activeSessionId);
            const sessionMemory = await getMemoryGateway().getSessionMemory(activeSessionId);
            const taskMemory = await getMemoryGateway().getTask(activeSessionId);

            const workingVal = getProjectionConsistencyValidator().validateWorkingProjection(workingMemory, sessionState);
            if (!workingVal.valid) {
              errors.push(`working-memory.md projection inconsistency: ${workingVal.reason}`);
            }

            const sessionVal = getProjectionConsistencyValidator().validateSessionProjection(sessionMemory, sessionState);
            if (!sessionVal.valid) {
              errors.push(`session-memory.md projection inconsistency: ${sessionVal.reason}`);
            }

            const taskVal = getProjectionConsistencyValidator().validateTaskProjection(taskMemory, sessionState);
            if (!taskVal.valid) {
