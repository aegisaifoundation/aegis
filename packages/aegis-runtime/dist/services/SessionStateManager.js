...
export class SessionStateManager {
    static instance = new SessionStateManager();
    static getInstance() {
     ...
     */
    async initializeSessionState(sessionId, actor = 'system') {
        const defaultState = {
            sessionId,
            status: 'ACTIVE',
            currentObjective: '',
     ...
     */
    async loadSessionState(sessionId, actor = 'system') {
     ...
     */
    async saveSessionState(state, txId, actor = 'system') {
        this.validateSessionState(state);
    ...
    async updateSessionState(sessionId, updates, actor = 'system') {
        ...
        try {
            ...
            const consistency = getProjectionConsistencyValidator().validateProjectionSynchronization(workingProj, updatedSessionProj, updatedTaskProj, mutated);
     ...
     */
    async refineStableFacts(facts, actor = 'system') {
        if (facts.length === 0)
            return [];
