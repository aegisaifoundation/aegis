...
const getMemoryTransactionManager = () => serviceRegistry.get('memoryTransactionManager');
const getProjectionGenerator = () => serviceRegistry.get('projectionGenerator');
const getMemoryWriteBuffer = () => serviceRegistry.get('memoryWriteBuffer');
const getMemoryObservability = () => serviceRegistry.get('MemoryObservability');
...
import { SessionLifecycleState, BootMode, RuntimeLockState, CheckoutStage } from '@aegis/sdk';
import { RuntimeSupervisorHooks } from './RuntimeSupervisorHooks.js';
import { calculateChecksum } from '../utils/fileHelpers.js';
...
export class RuntimeSessionManager {
    ...
    async initialize() {
        ...
        getMemoryWriteBuffer().startAutoFlush(5000);
        // Startup markings
    ...
    async recoverRuntime() {
        ...
        try {
            ...
            await getProjectionGenerator().projectSessionState(activeId);
        }
        catch (err) {
     ...
     */
    async createNewSession(tags = [], actor = 'system') {
        const wsRoot = path.dirname(workspaceManager.getWorkspacePath());
        const state = await runtimeStateManager.loadState();
        ...
        try {
            ...
            if (state.mountedSessionId) {
                const working = await getMemoryGateway().getWorkingMemory(state.mountedSessionId, actor);
                const workingFile = path.resolve(wsRoot, `memory/sessions/${state.mountedSessionId}/working-memory.md`);
                await getMemoryTransactionManager().registerWrite(txId, workingFile, working);
            }
            ...
            if (state.mountedSessionId) {
                ...
                const meta = await getMemoryGateway().loadSession(state.mountedSessionId, actor);
                meta.lifecycleState = SessionLifecycleState.INACTIVE;
                await getMemoryTransactionManager().registerWrite(txId, currentMetaFile, JSON.stringify(meta, null, 2));
            }
            ...
            const workingProj = getProjectionGenerator().generateWorkingMemoryProjection(defaultState);
            const sessionProj = getProjectionGenerator().generateSessionMemoryProjection(defaultState);
            await getMemoryTransactionManager().registerWrite(txId, newMetaFile, JSON.stringify(metaContent, null, 2));
            await getMemoryTransactionManager().registerWrite(txId, newHistoryFile, JSON.stringify({ messages: [], memoryVersion: '1.0.0' }, null, 2));
            await getMemoryTransactionManager().registerWrite(txId, path.join(sessionDir, 'session-state.json'), JSON.stringify(defaultState, null, 2));
            await getMemoryTransactionManager().registerWrite(txId, newSessionMemFile, sessionProj);
            await getMemoryTransactionManager().registerWrite(txId, newWorkingMemFile, workingProj);
            ...
            await getMemoryTransactionManager().registerWrite(txId, stateFile, JSON.stringify(state, null, 2));
            // Commit changes
            await getMemoryTransactionManager().commitTransaction(txId);
            // Update memory index registry list
            await getMemoryIndexManager().registerSession(metaContent);
    ...
    async checkoutSession(sessionId, actor = 'system') {
        ...
        try {
            ...
            const targetMeta = await getMemoryGateway().loadSession(sessionId, actor);
            ...
            await getMemoryTransactionManager().registerWrite(txId, targetMetaPath, JSON.stringify(targetMeta, null, 2));
            ...
            if (state.mountedSessionId) {
                const oldMeta = await getMemoryGateway().loadSession(state.mountedSessionId, actor).catch(() => null);
                if (oldMeta) {
                    await getMemoryIndexManager().registerSession(oldMeta);
                }
            }
            await getMemoryIndexManager().registerSession(targetMeta);
     ...
     */
    async forkSession(sessionId, actor = 'system') {
        const wsRoot = path.dirname(workspaceManager.getWorkspacePath());
        const sourceMeta = await getMemoryGateway().loadSession(sessionId, actor);
        ...
        const sourceState = await getMemoryGateway().getSessionState(sessionId, actor);
        const forkedState = {
            ...sourceState,
        ...
        await getMemoryIndexManager().registerSession(forkedMeta);
        await getMemoryManager().initialize(); // registry index refresh
        eventBus.emit(EventTypes.SESSION_FORKED, { sessionId: forkedSessionId, parentSessionId: sessionId }, 'session-continuity');
        return forkedSessionId;
    }
    async renameSession(sessionId, displayName, description, actor = 'system') {
        const meta = await getMemoryGateway().loadSession(sessionId, actor);
        ...
        await getMemoryIndexManager().registerSession(meta);
    ...
    async deleteSession(sessionId, actor = 'system') {
        ...
        if (sourceDir.includes('memory/sessions')) {
            await getMemoryManager().createSnapshot(sessionId, 'sessionMemory', actor).catch(() => { });
            await getMemoryManager().createSnapshot(sessionId, 'workingMemory', actor).catch(() => { });
        }
     ...
     */
    async resumeSession(sessionId, actor = 'system') {
        const wsRoot = path.dirname(workspaceManager.getWorkspacePath());
        const trashDir = path.resolve(wsRoot, `memory/trash/${sessionId}`);
    ...
    async archiveCurrentSession(actor = 'system') {
        const state = await runtimeStateManager.loadState();
        if (state.mountedSessionId) {
            await getMemoryManager().archiveSession(state.mountedSessionId, actor);
        }
    }
    async mountSession(sessionId) {
        await sessionMountManager.mount(sessionId);
    }
    async unmountCurrentSession() {
