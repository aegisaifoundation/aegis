import { StateConflictStrategy, StateError, StateErrorCode } from '@aegis/sdk';
import { StateVersionManager } from './StateVersionManager.js';
export class StateConflictResolver {
    static resolveConflict(localRecord, incomingMutation, strategy = StateConflictStrategy.REJECT) {
        if (strategy === StateConflictStrategy.REJECT) {
            throw new StateError(StateErrorCode.STATE_CONFLICT, `State conflict detected for key "${localRecord.key}" between local version ${localRecord.versionInfo.version} (${localRecord.versionInfo.originNodeId}) and remote mutation ${incomingMutation.mutationId} (${incomingMutation.originNodeId}). Strategy REJECT in effect.`, { key: localRecord.key, localRecord, incomingMutation });
        }
        if (strategy === StateConflictStrategy.LAST_WRITE_WINS) {
            const winner = StateVersionManager.evaluateLwwWinner({
                ...
                mutationId: localRecord.metadata?.lastMutationId || 'local',
        ...
        throw new StateError(StateErrorCode.STATE_CONFLICT, `State conflict detected for key "${localRecord.key}". Application conflict handler required.`);
    }
}
