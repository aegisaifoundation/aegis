import { TaskState, CoordinationError, CoordinationErrorCode } from '@aegis/sdk';
export class TaskStateValidator {
    static allowedMap = null;
    static getAllowedMap() {
        if (!this.allowedMap) {
    ...
    static validateTransition(taskId, currentState, targetState) {
        if (currentState === targetState)
            return;
        const allowed = this.getAllowedMap().get(currentState);
        if (!allowed || !allowed.has(targetState)) {
            throw new CoordinationError(CoordinationErrorCode.INVALID_TASK_STATE, `Invalid task state transition for "${taskId}": Cannot transition from ${currentState} to ${targetState}.`);
        }
    }
