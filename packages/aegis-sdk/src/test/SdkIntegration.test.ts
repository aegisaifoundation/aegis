...
import { AegisSDK, FeatureUnavailable, AegisError } from '../index.js';
...
describe('AEGIS SDK (ASDK) & AISCI Integration Tests', () => {
  ...
  test('Loopback Transport and engine routing', async () => {
    ...
    const mockInference = {
      generate: async (prompt: string) => ({ text: `Generated response to: ${prompt}`, model: 'llama-3' })
    };
  ...
  test('Event streaming subscriptions and notification', async () => {
    ...
    await aegis.subscribe('TrainingProgress', (payload: any) => {
