...
function createMockPackage(id, version, dependencies = {}) {
...
async function runTests() {
    ...
    if (!aInstalled)
        throw new Error('Test 2 Failed: engine-a not found in registry');
    ...
    if (!config.autoloadEngines.includes('engine-a'))
        throw new Error('Test 2 Failed: engine-a not in autoload list');
    ...
    if (!bInstalled)
        throw new Error('Test 3 Failed: engine-b not found in registry');
    ...
    if (aRemoved)
        throw new Error('Test 4 Failed: engine-a was not removed');
