...
export class TransactionManager {
  private currentJournal: TransactionJournal | null = null;
  private journalDir: string;
  private backupDir: string;

  constructor(private workspacePath: string, private db: PackageDatabase) {
  ...
  public async startTransaction(packageId: string, action: 'install' | 'remove' | 'update'): Promise<string> {
  ...
  public backupPath(originalPath: string): void {
  ...
  public trackAddedFile(filePath: string): void {
  ...
  public trackAddedDir(dirPath: string): void {
  ...
  public setOriginalConfig(config: any): void {
    if (this.currentJournal) {
  ...
  public updateState(state: TransactionJournal['state']): void {
    if (this.currentJournal) {
  ...
  public async recoverOrphanedTransactions(configPath: string): Promise<void> {
    ...
    for (const file of files) {
      if (file.endsWith('_journal.json')) {
        ...
        try {
          const journal: TransactionJournal = JSON.parse(fs.readFileSync(filePath, 'utf8'));
  ...
  private saveJournal(journal: TransactionJournal): void {
