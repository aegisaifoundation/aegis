...
const mockContext = {
    ...
    getLogger: () => ({
        info: (msg, src) => console.log(`    [${src}] ${msg}`),
        warn: (msg, src) => console.warn(`    [${src}] WARN: ${msg}`),
        error: (msg, src) => console.error(`    [${src}] ERROR: ${msg}`),
    }),
    getConfig: () => ({}),
    getSecrets: () => ({}),
    getService: (_) => undefined,
    getEventBus: () => ({
        on: () => { },
        off: () => { },
        emit: (ev, payload) => console.log(`    [EventBus] ${ev}:`, JSON.stringify(payload)),
    }),
};
async function runTests() {
    ...
    assert(healthOnline.details?.pid === engine.getPid(), 'Health report contains correct PID');
    // Test 5: Uptime
