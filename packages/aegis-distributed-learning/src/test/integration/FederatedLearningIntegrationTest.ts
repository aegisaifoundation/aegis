...
function buildMockDis() {
  const messageHandlers: Map<string, (payload: any, senderId: string) => void> = new Map();
  return {
    discoveryService: { discoverNodes: async () => ['mock-peer-1', 'mock-peer-2'] },
    capabilityService: {
      getRemoteCapabilities: async (_id: string) => ['federated_learning', 'distributed_learning'],
      advertiseCapabilities: async (_caps: string[]) => {}
    },
    trustService: { verifyPeerTrust: async (_id: string) => true },
    messagingService: {
      sendMessage: async (_targetId: string, type: string, payload: any) => {
        ...
        if (type === 'federated_round_start') {
          ...
          if (handler) handler({ roundId: payload.roundId, localWeights: { layer1: [0.1, 0.2], layer2: [0.3, 0.4] } }, _targetId);
        }
      },
      onMessage: (type: string, handler: (payload: any, senderId: string) => void) => {
        messageHandlers.set(type, handler);
      }
    },
    eventService: {
      publishEvent: async (_name: string, _payload: any) => {},
      subscribe: (_name: string, _handler: (payload: any) => void) => {}
    }
  };
}
