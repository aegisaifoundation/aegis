...
export class UnifiedMonitor {
    async getPlatformStatus() {
        ...
        if (serviceRegistry.has('engineManager')) {
            try {
                const engineMgr = serviceRegistry.get('engineManager');
        ...
        if (trainingScheduler && !trainingScheduler.isUnavailable) {
            try {
                ...
                activeJobs = queue.filter((j) => j.status === 'RUNNING').length;
            }
            catch { }
        }
        let activeRounds = 0;
        ...
        if (serviceRegistry.has('aegis-training-engine')) {
            try {
                const trainingEngine = serviceRegistry.get('aegis-training-engine');
