...
export async function runPlatformSimulation() {
    ...
    const mockLogger = {
        info: (msg) => console.log(`[Runtime Info] ${msg}`),
        warn: (msg) => console.log(`[Runtime Warn] ${msg}`),
        error: (msg) => console.log(`[Runtime Error] ${msg}`),
        log: (level, msg) => console.log(`[Runtime ${level}] ${msg}`)
    };
    ...
    await auip.start();
    const capRegistry = serviceRegistry.get('aegis-unified-platform:capability');
    const dashboard = serviceRegistry.get('aegis-unified-platform:dashboard');
    ...
    let ev = EventSpecification.createEvent('PackageInstalled', 'aegis-package-manager', { packageId: 'aegis-data' });
