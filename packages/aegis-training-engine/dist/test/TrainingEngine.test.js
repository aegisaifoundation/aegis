...
describe('AEGIS Training Engine (ATE) Tests', () => {
    ...
    test('Dataset Loading and splits creation', async () => {
        ...
        const splits = await serviceRegistry.get('aegis-training-engine:dataset').SplitDataset(datasetId, 0.5, 0.5, 0.0);
    ...
    test('Hyperparameter profile loading', () => {
        const hyper = serviceRegistry.get('aegis-training-engine:policy');
        assert.ok(hyper);
    });
...
//# sourceMappingURL=TrainingEngine.test.js.map