...
export class DatasetManager {
  ...
  private getAde(): any {
    if (serviceRegistry.has('aegis-data')) {
      return serviceRegistry.get<any>('aegis-data');
    }
    return null;
  }

  async LoadDataset(datasetId: string): Promise<{ datasetId: string; filePath: string; lineCount: number }> {
  ...
  async SplitDataset(
    ...
    valRatio = 0.1,
    testRatio = 0.1
  ): Promise<{ trainPath: string; valPath: string; testPath: string }> {
