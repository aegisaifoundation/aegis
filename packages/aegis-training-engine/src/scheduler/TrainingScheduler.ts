...
import { TrainingJob, TrainingConfig, JobStatus } from '../types/index.js';
...
export class TrainingScheduler {
  ...
  private activeJob: TrainingJob | null = null;
  ...
  private findJob(jobId: string): TrainingJob | undefined {
  ...
  private sortQueue() {
    this.queue.sort((a, b) => {
      ...
      const statusWeight = (s: JobStatus) => (s === 'RUNNING' || s === 'PAUSED' ? 2 : s === 'QUEUED' ? 1 : 0);
  ...
  private async processQueue() {
    ...
    try {
      ...
      await this.activeBackend.Prepare(
        nextJob.jobId,
        datasetSplitDir,
        nextJob.modelId,
        nextJob.config
