...
const getDiscovery = () => serviceRegistry.get<any>('distributed-intelligence:discovery');
const getMessaging = () => serviceRegistry.get<any>('distributed-intelligence:messaging');
...
export async function startApiServer() {
  ...
  server.listen(PORT, '127.0.0.1', () => {
    ...
    if (msgService) {
      ...
      const handleIncomingMessage = (payload: any, senderId: string, messageType: string) => {
      ...
      msgService.onMessage('hello_p2p', (payload: any, senderId: string) => {
        ...
        }).catch((err: any) => console.error('[P2P Reply Failed]', err.message));
      });

      msgService.onMessage('reply_p2p', (payload: any, senderId: string) => {
