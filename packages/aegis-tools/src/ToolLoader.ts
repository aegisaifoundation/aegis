...
import type { Tool, ToolContext } from '@aegis/runtime';
...
export class ToolLoader {
  private getAegisCoreRoot(): string {
    ...
    if (fs.existsSync(path.resolve(cwd, 'aegis/tools/package.json'))) {
      return path.resolve(cwd, '@aegis/tools');
    }
    return cwd;
  }
