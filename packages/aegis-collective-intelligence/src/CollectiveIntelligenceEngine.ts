...
export class CollectiveIntelligenceEngine implements IEngine {
  ...
  private distillationEngine!: KnowledgeDistillationEngine;
  private memory!: CollectiveMemory;
  private reasoningEvolution!: ReasoningEvolutionEngine;
  ...
  RecordExperience(
    ...
    const exp = this.experienceEngine.recordExperience(
      taskId, goal, reasoningTrace, toolsUsed, modelsUsed, policiesApplied,
      outcome, executionTimeMs, successScore, confidence, feedback
    );
  ...
  DistillKnowledge(
    ...
    if (!exp || !ref) throw new Error('Experience or Reflection not found');
  ...
  async PublishKnowledge(knowledgeId: string): Promise<boolean> {
    const ko = this.memory.getLatestKnowledge(knowledgeId);
    if (!ko) return false;
  ...
  Recommendations(prompt: string, domain: string) {
