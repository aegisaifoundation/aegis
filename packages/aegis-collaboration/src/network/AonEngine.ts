...
export class AonEngine {
  private tunnels = new Map<string, AonTunnel>();

  // A local keypair for Diffie-Hellman key exchange (X25519)
  private dh = crypto.createECDH('secp256k1');
   ...
   */
  async resolvePublicAddress(): Promise<PublicAddress> {
   ...
   */
  encryptMessage(tunnelId: string, plaintext: string): { ciphertext: string; iv: string; tag: string } {
