...
describe('AEGIS Data Engine Integration Tests', () => {
  ...
  before(async () => {
    ...
    await fsPromises.writeFile(
      path.join(sourceFolder, 'note1.txt'),
      'Medical clinical record. Doctor Jane Doe. Patient John Smith MRN-123456. Email is john.smith@hospital.com. Call at 555-0199.',
      'utf8'
    );
    await fsPromises.writeFile(
      path.join(sourceFolder, 'note2.md'),
      '# Clinical Summary\nNo critical findings reported. Policy holds true.',
      'utf8'
    );
  ...
  test('Data Source Manager CRUD operations', async () => {
    // Register
    const source = await dataEngine.RegisterSource(
      'src-folder-clinical',
      'Clinical Folder',
      'Folder',
      { path: sourceFolder }
    ...
    const disabled = serviceRegistry.get<any>('aegis-data:datasource').getSource('src-folder-clinical');
    assert.strictEqual(disabled.enabled, false);

    // Enable
    await dataEngine.EnableSource('src-folder-clinical');
    const enabled = serviceRegistry.get<any>('aegis-data:datasource').getSource('src-folder-clinical');
  ...
  test('Privacy Engine PII detection and redaction validation', async () => {
    ...
    const privacyEngine = serviceRegistry.get<any>('aegis-data:privacy');
