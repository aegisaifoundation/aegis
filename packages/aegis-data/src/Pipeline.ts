...
import { IProcessingPlugin } from './interfaces/IProcessingPlugin.js';
...
import { ProvenanceManager, ProvenanceRecord } from './ProvenanceManager.js';
...
export class DataProcessingPipeline {
  ...
  async run(
  ...
  ): Promise<{ samplesCount: number; language: string; data: string }> {
    ...
    await fs.writeFile(
      path.join(workspace.getSubdirPath('statistics'), 'summary.json'),
