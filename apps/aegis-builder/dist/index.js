...
function getBuilder(keysDirOption) {
...
program
    ...
    try {
        ...
        const parsedPkgs = rawPkgs.map((raw) => {
...
program
    ...
    try {
        const { builder } = getBuilder(options.keysDir);
        const outDir = path.resolve(options.outputDir);
        const parsedPkgs = options.pkgs ? options.pkgs.split(',').map((raw) => {
        ...
        const parsedBundles = options.bundles ? options.bundles.split(',').map((raw) => {
            const atIdx = raw.indexOf('@');
            const colonIdx = raw.indexOf(':');
            return {
                id: raw.substring(0, atIdx),
