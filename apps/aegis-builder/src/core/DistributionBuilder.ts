...
export interface BundleOptions {
  bundleId: string;
  version: string;
  packages: Array<{ id: string; version: string; path: string }>;
  outputDir: string;
  publisher: string;
}
...
export class DistributionBuilder {
  constructor(private signer: SignatureSigner) {}

  public async buildPackage(options: BuildOptions): Promise<string> {
  ...
  public async buildBundle(options: BundleOptions): Promise<string> {
  ...
  public async generateRepositoryIndex(repoDir: string): Promise<string> {
  ...
  private extractManifestFromPackage(aegPath: string): any {
    ...
    try {
      ...
      if (isWindows) {
        ...
        if (ext !== '.zip') fs.copyFileSync(aegPath, effectiveSrc);
        try {
          ...
          if (ext !== '.zip' && fs.existsSync(effectiveSrc)) fs.rmSync(effectiveSrc, { force: true });
        }
      } else {
  ...
  private extractBundleJsonFromBundle(bundlePath: string): any {
    ...
    try {
      ...
      if (isWindows) {
        ...
        if (ext !== '.zip') fs.copyFileSync(bundlePath, effectiveSrc);
        try {
          const result = spawnSync('powershell.exe', [
            '-NoProfile', '-NonInteractive', '-Command',
            `Expand-Archive -Path '${effectiveSrc}' -DestinationPath '${tempDir}' -Force`
  ...
  public async generateReleaseManifest(options: {
    releaseVersion: string;
    packages: Array<{ id: string; version: string; checksum: string }>;
    bundles: Array<{ id: string; version: string; checksum: string }>;
    outputDir: string;
  }): Promise<string> {
  ...
  private stripDevelopmentFiles(dirPath: string): void {
  ...
  private collectChecksumsRecursive(baseDir: string, currentDir: string, checksums: Record<string, string>): void {
  ...
  private archiveDirectory(srcDir: string, destZipPath: string): void {
    const isWindows = process.platform === 'win32';
    let result;
